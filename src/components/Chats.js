...
function Chats({ isAuth, setAuth }) {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      try {
        ...
        const response = await axios.get('https://hcholast.pythonanywhere.com/sessions', {
  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
      if (selectedSessionId) {
        try {
          const token = localStorage.getItem('token');
          const response = await axios.get(`https://hcholast.pythonanywhere.com//session/${selectedSessionId}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
  ...
  // Auto-scroll chat history to the bottom when new messages are added
  useEffect(() => {
  ...
  const handleStartNewSession = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('https://hcholast.pythonanywhere.com/start_session', {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`https://hcholast.pythonanywhere.com/chat/${selectedSessionId}`, { message }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteSession = async (sessionId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://hcholast.pythonanywhere.com/delete_session/${sessionId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-screen bg-gray-900">
      {/* Sidebar */}
      <div className="w-full md:w-64 bg-gray-800 shadow-lg p-4 flex-shrink-0 flex flex-col justify-between md:h-screen overflow-hidden">
        <h2 className="text-2xl font-semibold mb-4 text-white text-center">Previous Chats</h2>

        {/* Scrollable session list */}
        <div className="overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent mb-4 max-h-[calc(100vh-200px)] md:max-h-[70%]">
          {sessions.length > 0 ? (
      ...
      <div className="flex justify-end space-x-2 mb-4">
          <button
                ...
                className="text-gray-300 py-3  text-xl hover:text-white transition duration-200"
              >
                Home
              </button>
              ...
              <button
                ...
                className="bg-blue-500 text-white font-bold px-6 py-3 rounded-lg hover:bg-blue-600 transition duration-200"
              >
                Logout
              </button>
          </div>
        <div className="bg-gray-800 shadow-lg p-6 rounded-lg flex-grow flex flex-col min-h-0 overflow-hidden">
          <h2 className="text-3xl font-semibold text-white mb-6">Chat Session</h2>

          {/* Buttons above the chat window */}
